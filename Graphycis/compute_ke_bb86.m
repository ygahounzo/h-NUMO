...
kdm = 2;
idm = 101; jdm = 101;
dy = 10.0e3; dx = 10.0e3; % meters
mask = ones(jdm, idm); % no bathy
area = dx * dy * mask;
...
% save_folder = './BB86_Original_KE';
save_folder = './KE_nop/NUMO_KE_N4_v1';
% save_folder = './HYCOM_KE_mom4';
...
% name_root = sprintf('/Volumes/GLACIE/FALCON/N%d_Ne%d_v%d_%s/outbora',nop,Ne,nu,bc);
...
if strcmp(code,'numo')
    name_file = [name_root, sprintf('%04d', 0)];
...
for ifile = file_num_start:delta_file_num:file_num_end
    ...
    if strcmp(code,'bb86')
        name_data_file3 = [name_root, sprintf('%04d', ifile)];
        [dp,u,v, plonb6,platb6] = load_data_hycom(name_data_file3, idm, jdm, kdm);
...
if strcmp(code, 'bb86')
    ...
    file = [save_folder,sprintf('/ke_bb86_10kmv%d_%s', nu,bc)];
else
    file = [save_folder,sprintf('/ke_bb86_20kmv%d_%s', nu,bc)];
end
