...
subroutine openacc_enter_data()
  ...
  !mod_basis data
  !$acc update device(nglx,ngly,nglz,ngl,npts,dpsix,dpsiy,dpsiz,dpsix_tr,dpsiy_tr,dpsiz_tr) async
  !$acc update device(nqx,nqy,nqz,nq,dpsiqx,dpsiqy,dpsiqz) async

  !mod_grid
  !$acc enter data copyin(intma,face,intma_table) async

  !mod_initial
  !$acc enter data copyin(kvector,coriolis_constant) async
  !$acc enter data copyin(wjac, psih, dpsidx, dpsidy, indexq, wjac_df, psih_df, dpsidx_df, dpsidy_df, index_df) async
  !$acc enter data copyin(q_df_mlswe_init, pbprime_df, qb_df_mlswe_init) async
  !$acc enter data copyin(alpha_mlswe, pbprime_df_face, zbot_df, tau_wind_df, zbot_face, grad_zbot_quad) async
  !$acc enter data copyin(tau_wind, coriolis_df, coriolis_quad, fdt_bcl, fdt2_bcl, a_bcl, b_bcl) async

  !mod_metrics
  !$acc enter data copyin(ksi_x,ksi_y,ksi_z,eta_x,eta_y,eta_z,zeta_x,zeta_y,zeta_z,jac,xjac,massinv) async
  !$acc enter data copyin(ksiq_x,ksiq_y,ksiq_z,etaq_x,etaq_y,etaq_z,zetaq_x,zetaq_y,zetaq_z,jacq) async
  ...
  !mod_face
  !$acc enter data copyin(imapl, imapr, normal_vector, normal_vector_q) async

  !mod_bc
  !$acc enter data copyin(bc_count, bc_list) async

  !mod_variables
  !$acc enter data copyin(ope_ave, H_ave, Qu_ave, Qv_ave, Quv_ave, ope2_ave, ope2_ave_df, btp_mass_flux_ave, uvb_ave, ope2_face_ave) async
  !$acc enter data copyin(one_plus_eta_edge_2_ave, H_face_ave, tau_wind_ave, tau_bot_ave, uvb_face_ave) async
  !$acc enter data copyin(btp_mass_flux_face_ave, ope_face_ave, Qu_face_ave, Qv_face_ave, Quv_face_ave) async
  !$acc enter data copyin(uvb_ave_df, dpprime_visc, dpp_graduv, btp_dpp_graduv, pbprime_visc, dpp_uvp) async
