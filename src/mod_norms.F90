...
module mod_norms
    ...
    real,    dimension(:),   allocatable :: norm_inf_br, norm_inf_bu, norm_inf_bv, norm_inf_bw, norm_inf_bt, norm_inf_btr1, norm_inf_bene
...
contains
    ...
    subroutine mod_norms_create()
        ...
        allocate(norm_inf_br(nelem), norm_inf_bu(nelem), norm_inf_bv(nelem), norm_inf_bw(nelem), norm_inf_bt(nelem), norm_inf_btr1(nelem), norm_inf_bene(nelem), stat=AllocateStatus )
        if (AllocateStatus /= 0) stop "** Not Enough Memory to allocate local element norms - Mod_norms **"
    
        allocate(res_projected(3,nelem,npts), res_br(npts), res_bu(npts), res_bv(npts), res_bw(npts), res_bt(npts), res_btr1(npts), res_bene(npts), stat=AllocateStatus)
        if (AllocateStatus /= 0) stop "** Not Enough Memory to allocate local element residuals - Mod_norms **"
      ...
      subroutine mod_norms_get_residuals_infty_norms(ie, ndim, nelem, res_br, res_bu, res_bv, res_bw, res_bt, &
        ...
        real,    intent(out) :: norm_inf_br(nelem), norm_inf_bu(nelem), norm_inf_bv(nelem), norm_inf_bw(nelem), norm_inf_bt(nelem)
