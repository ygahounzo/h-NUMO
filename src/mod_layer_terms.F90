...
module mod_layer_terms
    ...
    public :: layer_mass_advection_terms, evaluate_dp, evaluate_dp_face, &
            ...
            evaluate_mom_v3, evaluate_bcl_v3, evaluate_consistency_face, layer_pressure_terms_v1

    contains

    subroutine layer_mass_advection_terms(qprime,qprime_face)
        ...
        do iface = 1,nface
            ...
            do k = 1,nlayers
                ...
                do iquad = 1,nq
                    ...
                    if(uu*nxl > 0.0) then 
                        flux_edge_bcl(1,iquad,iface,k) = uu * dpl
                    else 
                        flux_edge_bcl(1,iquad,iface,k) = uu * dpr 
                    endif 

                    if(vv*nyl > 0.0) then 
                        flux_edge_bcl(2,iquad,iface,k) = vv * dpl
                    else 
                        flux_edge_bcl(2,iquad,iface,k) = vv * dpr 
                    endif 

                    sum_layer_mass_flux_face(1,iquad,iface) = sum_layer_mass_flux_face(1,iquad,iface) + flux_edge_bcl(1,iquad,iface,k)
                    sum_layer_mass_flux_face(2,iquad,iface) = sum_layer_mass_flux_face(2,iquad,iface) + flux_edge_bcl(2,iquad,iface,k)
                end do 
    ...
    subroutine consistency_mass_terms(qprime, qprime_face, flux_deficit_mass_face)
        ...
        use mod_grid, only:  npoin_q, intma_dg_quad,nface,face, npoin, intma
        use mod_initial, only: pbprime, pbprime_face
        use mod_input, only: nlayers
        use mod_variables, only: btp_mass_flux_ave, ope_face_ave, sum_layer_mass_flux, flux_adjustment, flux_adjust_edge
        use mod_face, only: normal_vector_q
    ...
    subroutine layer_momentum_advec_terms(q, qprime)
    ...
    subroutine layer_momentum_advec_terms_upwind(q, qprime)
    ...
    subroutine layer_momentum_advec_terms_upwind_v1(q_df, qprime)
    ...
    end subroutine layer_momentum_advec_terms_upwind_v1
    ...
    subroutine layer_windbot_stress_terms(qprime)
        ...
        use mod_constants, only : gravity
        use mod_variables, only: tau_bot_ave, tau_wind_ave, tau_wind_int, tau_bot_int
    ...
    subroutine evaluate_bcl(q, qprime, q_df, qprime_df, qb_df)
        ...
        real, dimension(3,npoin_q,nlayers), intent(inout) :: q, qprime
        real, dimension(4,npoin), intent(in) :: qb_df
        
        integer :: I, k, el, er, n , iface, il, jl, kl, Iq
        real :: uv_df(2,npoin,nlayers)

        call velocity_df1(uv_df, q_df, qb_df)
        ...
        call evaluate_mom_v1(q, qprime, q_df, qprime_df)

    end subroutine evaluate_bcl

    subroutine evaluate_bcl_v1(qprime, q_df, qprime_df, qb_df)
        ...
        call evaluate_mom_v2(qprime,qprime_df)

    end subroutine evaluate_bcl_v1

    subroutine evaluate_bcl_v2(qprime, qprime_face, q_df, qprime_df, qb_df)
    ...
    end subroutine evaluate_bcl_v2

    subroutine evaluate_bcl_v3(q_df, qprime_df, qb_df)
    ...
    end subroutine evaluate_bcl_v3

    subroutine velocity_df1(uv_df, q_df, qb_df)
    ...
    subroutine evaluate_mom_v1(q, qprime, q_df, qprime_df)
    ...
    end subroutine evaluate_mom_v1

    subroutine evaluate_mom_v2(qprime,qprime_df)
    ...
    end subroutine evaluate_mom_v2

    subroutine evaluate_mom_v3(qprime,qprime_face,qprime_df)
    ...
    end subroutine evaluate_mom_v3
    ...
    subroutine evaluate_mom_face_v1(q_face, qprime_face, q, qprime)
        ...
        do iface = 1, nface
            ...
            do iquad = 1, nq
                ...
                q_face(2:3,1,iquad,iface,:) = q(2:3,I,:)
                qprime_face(2:3,1,iquad,iface,:) = qprime(2:3,I,:)
                ...
                if(er > 0) then
                    ...
                    q_face(2:3,2,iquad,iface,:) = q(2:3,I,:)
                    qprime_face(2:3,2,iquad,iface,:) = qprime(2:3,I,:)
