...
contains
   ...
   subroutine restart_swe(qb_df, fname)
      ...
      ! Read the restart file
      call read_swe(qb_df_read,fname)

      ! Barotropic variables at the dofs (nodal points)

      qb_df(1,:) = (gravity/alpha_mlswe(1))*qb_df_read(1,:)
      qb_df(3,:) = qb_df_read(2,:)*qb_df(1,:)
      qb_df(4,:) = qb_df_read(3,:)*qb_df(1,:)
      qb_df(2,:) = qb_df(1,:) - pbprime_df(:)

   end subroutine restart_swe
   ...
   subroutine read_swe(qb_df,fname)
      ...
      if(ig_rank==0) then
         ...
         if (pos > 0 .and. pos + 2 == len_trim(fname)) then
            call load_data_swe_nc(fname, qb_grp, npoin_grp)
         end if
      ...
      if(ig_rank==0) then
         deallocate(qb_grp) !not the best idea, but will work for now
         deallocate(npoin_l_grp,nelem_l_grp)
      end if
   ...
   subroutine load_data_swe_nc(name_data_file, qb_df_g, npoin_g)

      use netcdf

      character(len=*), intent(in) :: name_data_file
      real, dimension(3, npoin_g), intent(out) :: qb_df_g
      integer, intent(in) :: npoin_g

      integer :: ncid, time_dimid, npoin_dimid, nlayers_dimid, zi_dimid
      integer :: dt_btp_varid, x_varid, y_varid
      integer :: h_varid, ub_varid, vb_varid
      integer :: status
      integer :: time_size, npoin_size, nlayers_size, zi_size
      real(8), allocatable :: dt_btp(:)
      real(8), allocatable :: h(:), ub(:), vb(:)
      ...
      ! Allocate arrays based on the dimensions
      allocate(h(npoin_g), ub(npoin_g), vb(npoin_g))

      ! Get variable IDs
      call check(nf90_inq_varid(ncid, "h", h_varid))
      call check(nf90_inq_varid(ncid, "ub", ub_varid))
      call check(nf90_inq_varid(ncid, "vb", vb_varid))

      ! Read the variables
      call check(nf90_get_var(ncid, h_varid, h))
      call check(nf90_get_var(ncid, ub_varid, ub))
      call check(nf90_get_var(ncid, vb_varid, vb))
   ...
   end subroutine load_data_swe_nc
