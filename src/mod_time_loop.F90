...
contains
   ...
   subroutine time_loop()
      ...
      use mod_initial, only: q_df_mlswe_init, qb_df_mlswe_init, alpha_mlswe
      use mod_input, only: dt,dt_btp,time_initial, time_final, time_restart, &
         fname_root, lprint_diagnostics, &
         nlayers, is_mlswe, ti_method_btp, dump_data, lcheck_conserved
      use mod_constants, only: gravity
      use mod_restart, only: restart_mlswe, restart_swe
      use mpi
      ...
      if (time_initial == 0) then
         ...
         if(dump_data) then
            ...
            elseif(trim(out_type) == 'nc') then
               if (nlayers > 1) then
                  ...
                  call diagnostics_btp_nc(qb0_df_mlswe(1:4,:),inorm,idone)
               endif
         ...
         if(trim(out_type) == 'txt' .or. trim(out_type) == 'nc') then
            ...
            if (nlayers > 1) then
               fnp=trim(fname_root) // trim(fnp4)
               if(trim(out_type) == 'nc') fnp = trim(fname_root) // trim(fnp4)//trim('.nc')
               ...
               call restart_swe(qb0_df_mlswe, fnp)
            endif
      ...
      if(lcheck_conserved) then
         ...
         if (nlayers > 1) then
            ...
            fnps = trim('mass_')//trim(fname_root)//trim('.cons')
            ...
            hb = (alpha_mlswe(1)/gravity)*qb0_df_mlswe(1,:)
            call compute_conserved(mass_conserv_l,hb)

            mass_conserv_g = 0.0
            call mpi_reduce(mass_conserv_l,mass_conserv_g,1,MPI_PRECISION, &
               mpi_sum,0,mpi_comm_world,ierr)

            mass_conserv0_g(1) = mass_conserv_g
         endif
      ...
      if (lprint_diagnostics) then
         if (nlayers > 1) then
            ...
            call print_diagnostics_swe(qb0_df_mlswe(1:4,:),time,itime,dt_btp,idone,mass_conserv0_g,ntime,fnp11,unit0)
         endif
      end if
      ...
      do while (time < time_final)
         ...
         if (nlayers > 1) then
            ...
            call ti_rk_btp(q0_df_mlswe, qb0_df_mlswe)
         endif
