module mod_rk_mlswe
    ...
    do mstep = 1, N_btp
        ...
        do ik = 1, kstages
            ...
            inv_pb1 = 1.0 / qb1_df(1,:)

            ope2_ave_df    = ope2_ave_df + (1.0 + qb1_df(2,:)/pbprime_df)**2
            uvb_ave_df(1,:) = uvb_ave_df(1,:) + qb1_df(3,:) * inv_pb1
            uvb_ave_df(2,:) = uvb_ave_df(2,:) + qb1_df(4,:) * inv_pb1

            call create_rhs_btp(rhs, qb1_df, qprime_df)

            ! Update barotropic variables using SSPRK formula
            qb_df(2:4,:) = a0*qb0_df(2:4,:) + a1*qb1_df(2:4,:) + a2*qb2_df(2:4,:) + dtt*rhs
