module mod_rk_mlswe
    ...
    subroutine ti_barotropic_ssprk_mlswe_v0(qb_df,qprime_df)
        ...
        do mstep = 1, N_btp
            ...
            do ik=1,kstages
                ...
                ope2_ave_df = ope2_ave_df + (1.0 + qb1_df(2,:)/pbprime_df(:))**2
    ...
    subroutine ti_barotropic_ssprk_mlswe(qb_df, qprime_df)
        ...
        use mod_grid,             only: npoin, npoin_q, nface
        use mod_basis,            only: nqx, nqy, nqz, nq
        use mod_input,            only: nlayers, dt_btp, kstages, method_visc  ! [OPT] removed duplicate nlayers
        use mod_rhs_btp,          only: create_rhs_btp
        use mod_barotropic_terms, only: btp_mom_boundary_df
        use mod_variables,        only: one_plus_eta_edge_2_ave, ope_ave, H_ave, Qu_ave, Qv_ave, Quv_ave, &
                                        ope2_ave, ope2_ave_df, uvb_face_ave, btp_mass_flux_face_ave, &
                                        ...
                                        one_plus_eta_out, tau_wind_ave, tau_bot_ave, &
                                        btp_mass_flux_ave, uvb_ave, uvb_ave_df, ope2_face_ave
        use mod_variables,        only: graduvb_face_ave, graduvb_ave, rhs_btp
        ...
        ! real, dimension(3,npoin) :: rhs_btp
        real, dimension(4,npoin) :: qb0_df, qb1_df, qb2_df
        real, dimension(npoin)   :: inv_pb1          ! [OPT] precomputed reciprocal of qb1_df(1,:)
        integer :: mstep, ik                         ! [OPT] removed I, Iq, iquad, iface, ilr, k (unused)
        real    :: N_inv, a0, a1, a2, dtt            ! [OPT] removed beta (unused); a0-a2 now actually used

        ! Zero-initialise all accumulation buffers
        one_plus_eta_edge_2_ave = 0.0;  uvb_ave             = 0.0
        uvb_ave_df              = 0.0;  ope_ave             = 0.0
        btp_mass_flux_ave       = 0.0;  H_ave               = 0.0
        Qu_ave                  = 0.0;  Qv_ave              = 0.0
        Quv_ave                 = 0.0;  ope2_ave_df         = 0.0
        uvb_face_ave            = 0.0;  ope_face_ave        = 0.0
        ope2_face_ave           = 0.0;  btp_mass_flux_face_ave = 0.0
        H_face_ave              = 0.0;  Qu_face_ave         = 0.0
        Qv_face_ave             = 0.0;  Quv_face_ave        = 0.0
        tau_wind_ave            = 0.0;  tau_bot_ave         = 0.0
        ope2_ave                = 0.0;  graduvb_face_ave    = 0.0
        graduvb_ave             = 0.0
        ...
        do mstep = 1, N_btp
            ...
            do ik = 1, kstages
                ...
                ! Compute reciprocal of qb1_df(1,:)
                inv_pb1 = 1.0 / qb_df(1,:)

                ope2_ave_df    = ope2_ave_df + (1.0 + qb_df(2,:)/pbprime_df)**2
                uvb_ave_df(1,:) = uvb_ave_df(1,:) + qb_df(3,:) * inv_pb1
                uvb_ave_df(2,:) = uvb_ave_df(2,:) + qb_df(4,:) * inv_pb1
