...
subroutine filter(q)
    ...
    !
    !     !Initialize
    !     qq_i=0; qq_ij=0; qq_ijk=0
    !
    !     !Filter Along i direction
    !     do k=1,ngl
    !        do j=1,ngl
    !           do i=1,ngl
    !              do l=1,ngl
    ...
    !                ip1=intma(l,j,k,1)
    !                 do m=1,nvar
    !                    qq_i(m,ip)=qq_i(m,ip) + fx(i,l)*qq(m,ip1)
    !                 end do !m
    !              end do !l
    !           end do !i
    !        end do !j
    ...
    !                 ip1=intma(i,l,k,1)
    !                 do m=1,nvar
    !                    qq_ij(m,ip)=qq_ij(m,ip) + fy(j,l)*qq_i(m,ip1)
    !                 end do !m
    !              end do !l
    !           end do !i
    !        end do !j
    ...
    !                 ip1=intma(i,j,l,1)
    !                 do m=1,nvar
    !                    qq_ijk(m,ip)=qq_ijk(m,ip) + fz(k,l)*qq_ij(m,ip1)
    !                 end do !m
    !              end do !l
    !           end do !i
    !        end do !j
    ...
    !              !Store RHS
    !              do m=1,nvar
    !                 b(m,ip)=qq_ijk(m,i)!b(m,ip) + wq*qq_ijk(m,i)
    !              end do !m
    !     end do !k
    !
    !  end do !ie
...
!>@date 11/2009 Extended to 3D
!----------------------------------------------------------------------!
subroutine filter_limiter_dss(q)
    ...
    use mod_interface, only: compute_local_gradient_limiter_dss
    ...
    do e=1,nelem
        ...
        !Construct Local Derivatives for Prognostics Variables
        call compute_local_gradient_limiter_dss(fqf,qq,nglx,ngly,nglz,ndim)
...
end subroutine filter_limiter_dss
