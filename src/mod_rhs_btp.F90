...
contains
    ...
    subroutine create_rhs_btp3(rhs,qb_df,qprime_df,stage)
        ...
        !call create_rhs_btp_volume_qdf4(rhs, qb_df, qprime_df)
        ...
        if(method_visc > 0) call btp_create_laplacian(rhs_visc_btp,qb_df)
        !if(stage == 1) qbface_ave = qb_df_face
        qbface_ave = qbface_ave + qb_df_face
    ...
    subroutine create_rhs_btp2(rhs,qb_df,qprime,stage)
        ...
        call create_rhs_btp_volume_qdf2(rhs, qb_df, qprime)

        call btp_create_postcommunicator(qb_df_face,4)

        call creat_btp_fluxes_qdf2(rhs,qb_df_face)
    ...
    subroutine create_rhs_btp_volume_qdf3(rhs, qb_df, qprime_df)
        ...
        real :: source_x, source_y, Hq, quux, quvxy, qvvy
        real :: wq, hi, dhdx, dhdy, coef_fric, tau_bot_u, tau_bot_v, ope, &
                dp, dpp, udp, vdp, ub, vb, Pstress, Pbstress, ubot, vbot, speed, tmp
        ...
        do Iq = 1, npoin
            ...
            if (botfr == 1) then
                ...
                speed = (cd_mlswe/gravity)*qprime_df(1,Iq,nlayers)
                tau_bot_u = speed*ubot
                tau_bot_v = speed*vbot
            elseif (botfr == 2) then
                ubot = qprime_df(2,Iq,nlayers) + ub
    ...
    subroutine create_rhs_btp_volume_qdf2(rhs, qb_df, qprime)
    ...
    subroutine create_rhs_btp_volume_qdf4(rhs, qb_df, qprime_df)

        use mod_grid, only : npoin_q, npoin, nelem, intma_dg_quad, intma
        use mod_basis, only: npts
        use mod_constants, only: gravity
        use mod_initial, only: grad_zbot_quad, tau_wind, psih, dpsidx,dpsidy, indexq, wjac, &
                                coriolis_quad, one_over_pbprime, alpha_mlswe, coriolis_df
        use mod_variables, only: Q_uu_dp, Q_uv_dp, Q_vv_dp, H_bcl
        use mod_input, only: nlayers, cd_mlswe, botfr
        use mod_variables, only: Q_uu_dp_df, Q_uv_dp_df, Q_vv_dp_df, H_bcl_df
        ...
        real :: wq, hi, dhdx, dhdy, coef_fric, tau_bot_u, tau_bot_v, ope, &
                dp, dpp, udp, vdp, ub, vb, Pstress, Pbstress, ubot, vbot, speed
        real :: pp, up, vp, quue, quve, qvve, hqe, cq
        ...
        do Iq = 1, npoin_q
            ...
            dp = 0.0; dpp = 0.0; udp = 0.0; vdp = 0.0
            pp = 0.0; up = 0.0; vp = 0.0; cq = 0.0
            hqe = 0.0; quue = 0.0; quve = 0.0; qvve = 0.0
            ...
            do ip = 1,npts
                ...
                hqe = hqe + hi*H_bcl_df(I)
                quue = quue + hi*Q_uu_dp_df(I)
                quve = quve + hi*Q_uv_dp_df(I)
                qvve = qvve + hi*Q_vv_dp_df(I)
                cq = cq + hi*coriolis_df(I)
            end do
            ...
            source_x = cq*vdp + gravity*(tau_wind(1,Iq) - tau_bot_u) - &
                        gravity*dp*grad_zbot_quad(1,Iq)

            source_y = -cq*udp + gravity*(tau_wind(2,Iq) - tau_bot_v) - &
                        gravity*dp*grad_zbot_quad(2,Iq)

            ope = 1.0 + dpp * one_over_pbprime(Iq)
            Hq = (ope**2) * hqe

            quux = ub * udp + ope * quue
            quvxy = ub * vdp + ope * quve
            qvvy = vb * vdp + ope * qvve 
    ...
    end subroutine create_rhs_btp_volume_qdf4
    ...
    subroutine creat_btp_fluxes_qdf(rhs,qb_df_face)
        ...
        real :: qbl(4,nq), qbr(4,nq), lam1, lam2
        ...
        do iface = 1, nface
            ...
            do iquad = 1, nq
                ...
                lam1 = nxl*ul(n) + nyl*vl(n)
                lam2 = nxr*ur(n) + nyr*vr(n)
                lamb = min(abs(lam1),abs(lam2))
    ...
    subroutine creat_btp_fluxes_qdf2(rhs,qb_df_face)
        ...
        real, dimension(nq) :: quu, quv, qvu, qvv, H_face_temp, flux_edge_x, flux_edge_y, one_plus_eta_edge
        real, dimension(nq) :: ul, ur, vl, vr
        real :: pU_L, pU_R, lamb, dispu, dispv, pbpert_edge, dispp
        real :: qbl(4,nq), qbr(4,nq), lam1, lam2
        ...
        do iface = 1, nface
            ...
            do iquad = 1, nq
                ...
                !lamb = 2.0*coeff_mass_pbpert_LR(iquad,iface)
                lam1 = abs(nxl*ul(iquad) + nyl*vl(iquad)) + sqrt(alpha_mlswe(nlayers) * pbprime_face(1,iquad,iface))
                lam2 = abs(nxr*ur(iquad) + nyr*vr(iquad)) + sqrt(alpha_mlswe(nlayers) * pbprime_face(2,iquad,iface))
                lamb = max(lam1,lam2)
    ...
    subroutine creat_btp_fluxes_qdf3(rhs,qb_df_face)
        ...
        use mod_initial, only: coeff_pbpert_L_df, coeff_pbub_LR_df, coeff_pbpert_R_df, &
                                ...
                                pbprime_df, zbot_df, alpha_mlswe, pbprime_df_face
        use mod_constants, only: gravity
        ...
        real :: pU_L, pU_R, lamb, dispu, dispv, pbpert_edge, dispp
        real :: qbl(4,ngl), qbr(4,ngl), H, lam1, lam2
        ...
        do iface = 1, nface
            ...
            do n = 1,ngl
                ...
                pbpert_edge = 0.5*(qbl(2,n) + qbr(2,n))
                ...
                flux_edge_x(n) = 0.5*(qbl(3,n) + qbr(3,n))
                flux_edge_y(n) = 0.5*(qbl(4,n) + qbr(4,n))
            ...
            do m = 1, ngl
                ...
                wq = jac_face(m,1,iface)
                nxl = normal_vector(1,m,1,iface)
                nyl = normal_vector(2,m,1,iface)

                nxr = -nxl ; nyr = -nyl

                H_kx = nxl*H_face_temp(m)
                H_ky = nyl*H_face_temp(m)
                
                !lamb = 2.0*coeff_mass_pbpert_LR_df(m,iface)
                lam1 = abs(nxl*ul(m) + nyl*vl(m)) + sqrt(alpha_mlswe(nlayers) * pbprime_df_face(1,m,iface))
                lam2 = abs(nxr*ur(m) + nyr*vr(m)) + sqrt(alpha_mlswe(nlayers) * pbprime_df_face(2,m,iface))
                lamb = max(lam1,lam2)

                dispu = 0.5*lamb*(qbr(3,m) - qbl(3,m))
                dispv = 0.5*lamb*(qbr(4,m) - qbl(4,m))

                flux_x = nxl*quu(m) + nyl*quv(m) - dispu
                flux_y = nxl*qvu(m) + nyl*qvv(m) - dispv
                dispp = 0.5*lamb*(qbr(2,m) - qbl(2,m))
                flux = nxl*flux_edge_x(m) + nyl*flux_edge_y(m) - dispp
