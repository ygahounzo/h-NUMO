...
contains
    ...
    subroutine create_rhs_btp_volume_qdf(rhs, qb_df, qprime_df)
        ...
        do concurrent (Iq = 1:npoin_q)
            ...
            sc_x =  coriolis_quad(Iq) * vdp + gravity * (tau_wind(1,Iq) - tb_u) - gravity * dp * grad_zbot_quad(1,Iq)
            sc_y = -coriolis_quad(Iq) * udp + gravity * (tau_wind(2,Iq) - tb_v) - gravity * dp * grad_zbot_quad(2,Iq)
    ...
    subroutine create_btp_fluxes_qdf(rhs,qb,qprime_df)
        ...
        do concurrent (iface = 1:nface, iquad = 1:nq)
            ...
            !------------------------------------------------------------
            ! 2) Scatter to rhs (race if faces share I!)
            !------------------------------------------------------------
            ! do iquad = 1, nq
                wq = jac_faceq(iquad,1,iface)
