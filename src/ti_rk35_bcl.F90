subroutine ti_rk35_bcl(q_df,qb_df,qp_df_out)
	...
	use mod_rk_mlswe, only: ti_rk35_btp, ti_barotropic_rk_mlswe2, ti_barotropic_rk_mlswe3, ti_barotropic_ssprk_mlswe
    use mod_barotropic_terms, only: btp_evaluate_mom_dp, btp_evaluate_mom_dp_face, btp_extract_df_face, btp_interpolate_face
    use mod_layer_terms, only: evaluate_mom_mass, evaluate_mom_mass_face, bcl_extract_df_face, &
							bcl_extract_dp_df_face, bcl_interpolate_face, bcl_dp_face, bcl_interpolate_face_v1
    ...
    real, dimension(npoin,nlayers) :: dpprime_df2, dpprime_df
	...
	real, dimension(1,2,ngl,nface,nlayers) :: dp_df_face
	real, dimension(3,2,ngl,nface,nlayers) :: q_df_face
	...
	! Extract face values for the nodal points and communicate them to the neighboring processors.
	...
	! Extract face values bcl 
	call bcl_extract_df_face(q_df_face, q_df)
	call bcl_create_communicator(q_df_face,3,nlayers,ngl)

	call bcl_interpolate_face(q_face, qprime_face, q_df_face, qb_face)
	...
	call ti_rk35_btp(one_plus_eta,one_plus_eta_edge_2_ave,uvb_ave, ope_ave, ope2_ave, &
	...
	! Extract face values bcl q_df_face (dp, udp, vdp at the dofs)
	call bcl_extract_df_face(q_df_face, q_df2)

	! Communicate the q_df_face vlaues within the processors 
	call bcl_create_communicator(q_df_face,3,nlayers,ngl)

	! Interpolate to the quad points 
	call bcl_interpolate_face(q_face1, qprime_face3, q_df_face, qbp_face)

    ! Take the average between the predition and the previous time step values
	qprime_avg = 0.5*(qprime2 + qprime)
	qprime_face_avg = 0.5*(qprime_face3 + qprime_face)
	...
	! Communication of dpprime face values within the processor boundary
	call bcl_extract_dp_df_face(dp_df_face,q_df)
	call bcl_create_communicator(dp_df_face,1,nlayers,ngl)

	call bcl_dp_face(dprime_face_corr, dp_df_face)

	qprime_df_avg(1,:,:) = 0.5*(qprime_df(1,:,:) + dpprime_df2(:,:))
	qprime_avg(1,:,:) = 0.5*(qprime(1,:,:) + qprime_avg(1,:,:))
	qprime_face_avg(1,:,:,:,:) = 0.5*(qprime_face(1,:,:,:,:) + dprime_face_corr(:,:,:,:))

	call momentum_v2(q,qprime_avg,q_df,q_face,qprime_face_avg,qb,ope_ave,one_plus_eta_edge_2_ave,uvb_ave,u_edge,v_edge,Qu_ave,&
		Qv_ave,Quv_ave,H_ave,uvb_face_ave,ope_face_ave,Qu_face_ave,Qv_face_ave,Quv_face_ave,H_face_ave,&
		qprime_df_avg,ope_ave_df,tau_bot_ave,tau_wind_ave, qprime_face2,flag_pred,&
		q2,q_face2,qb_df,qprime2,qprime_face, ope2_ave,uvb_df_ave, dpprime_df)
