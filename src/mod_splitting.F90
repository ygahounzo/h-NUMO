...
contains
    ...
    subroutine create_rhs_bcl(G, inp, b, mf, par, btp, bcl, init, ref, mpic, tsp, mt, rhs, qprime_df, q_df)
        ...
        ! Pre-scale dpprime_visc and dpp_graduvw by A_H = (visc + μ_SGS_prev) so the
        ! Laplacian assembles ∇·(A_H×dp×∇u) with {A_H×dp} face averaging (DG-correct form).
        ...
        ! ∇²z_k needed for APE and/or thickness diffusion.
        ...
        !
        ! Dyn-SGS path: rhs_visc_bcl = massinv*∇·((visc+μ_prev)×dp×∇u), pre-scaled above.
        !   Step A: apply massinv to all rhs rows (gives ∂q/∂t|_inv at each node).
        !   Step B: compute μ_SGS per element from the mass residual rhs(1,:,k).
        !   Step C: add pre-scaled rhs_visc_bcl to momentum rows (viscosity already baked in).
        !
        ! Standard path: rhs_visc_bcl = (visc+nu_smag)*massinv*L(q) already scaled.
        if (inp%lDyn_SGS) then
            ...
            ! Step C: add massinv*∇·((visc+μ)×dp×∇u) to momentum rows (viscosity pre-baked).
            !$acc parallel loop gang collapse(2) &
