...
module mod_initial_mlswe
    ...
    !------------------------------------------------------------------!
    subroutine find_dry_elements(G, inp, b, tsp, ope2_ave_df, qprime_df, alpha, dry_flg)
        ...
        real,    dimension(G%npoin),              intent(in)  :: ope2_ave_df
        ...
        do ie = 1, G%nelem
            ...
            do k = 1, inp%nlayers
                ...
                do ip = 1, b%npts
                    I = tsp%indexq(ip, Iq)
                    dp_node = sqrt(ope2_ave_df(I)) * qprime_df(1, I, k)
